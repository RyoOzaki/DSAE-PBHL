...
class DSAE(object):
    ...
    def get_encoder_weight(self, indexies=None):
        assert self._sess is not None, "Session is not initialized!! Please call init_session or set_session."
        if indexies is None:
            tf_weights = self._tf_encode_weight
        else:
            if type(indexies) not in (np.ndarray, list, tuple):
                indexies = (indexies, )
            tf_weights = [self._tf_encode_weight[idx] for idx in indexies]
        return self._sess.run(tf_weights)

    def get_encoder_biases(self, indexies=None):
        assert self._sess is not None, "Session is not initialized!! Please call init_session or set_session."
        if indexies is None:
            tf_biases = self._tf_encode_bias
        else:
            if type(indexies) not in (np.ndarray, list, tuple):
                indexies = (indexies, )
            tf_biases = [self._tf_encode_bias[idx] for idx in indexies]
        return self._sess.run(tf_biases)

    def get_decoder_weight(self, indexies=None):
        assert self._sess is not None, "Session is not initialized!! Please call init_session or set_session."
        if indexies is None:
            tf_weights = self._tf_decode_weight
        else:
            if type(indexies) not in (np.ndarray, list, tuple):
                indexies = (indexies, )
            tf_weights = [self._tf_decode_weight[idx] for idx in indexies]
        return self._sess.run(tf_weights)

    def get_decoder_biases(self, indexies=None):
        assert self._sess is not None, "Session is not initialized!! Please call init_session or set_session."
        if indexies is None:
            tf_biases = self._tf_decode_bias
        else:
            if type(indexies) not in (np.ndarray, list, tuple):
                indexies = (indexies, )
            tf_biases = [self._tf_decode_bias[idx] for idx in indexies]
        return self._sess.run(tf_biases)
